...
static void on_ble_write(dk_ble_gyro_service_t * p_gyro_service, ble_evt_t const * p_ble_evt)
{
	if(p_ble_evt->evt.gatts_evt.params.write.handle == p_gyro_service->gyro_raw_char_handles.cccd_handle) {
		...
		err_code = sd_ble_gatts_value_get(p_gyro_service->conn_handle, p_gyro_service->gyro_raw_char_handles.cccd_handle, &rx_data);
		...
		rx_data.offset = 0;
		rx_data.p_value = dk_ble_gyro_evt.data;
		err_code = sd_ble_gatts_value_get(p_gyro_service->conn_handle, p_gyro_service->gyro_alert_char_handles.cccd_handle, &rx_data);
		APP_ERROR_CHECK(err_code);
		if(dk_ble_gyro_evt.data[0] == 0){
		...
		rx_data.offset = 0;
		rx_data.p_value = dk_ble_gyro_evt.data;
		err_code = sd_ble_gatts_value_get(p_gyro_service->conn_handle, p_gyro_service->gyro_config_char_handles.value_handle, &rx_data);
		APP_ERROR_CHECK(err_code);
...
uint32_t dk_ble_gyro_config_write(dk_ble_gyro_service_t * p_gyro_service, uint8_t * p_data)
{
	...
	return sd_ble_gatts_value_set(p_gyro_service->conn_handle, p_gyro_service->gyro_config_char_handles.value_handle, &ble_gatts_value);
}
