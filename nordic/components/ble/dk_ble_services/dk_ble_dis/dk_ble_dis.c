...
ret_code_t dk_ble_dis_init(void)
{
    ...
    char device_ser_number[((sizeof(serial_number)) * 2) +
                           1];   // Sizeof device serial number (1 byte = 2 hex chars) and string terminator
    char device_model_number[DECIMAL_DIGITS_MAX(hw_info.hw_id) + DECIMAL_DIGITS_MAX(hw_info.hw_v) +
                             2]; // Sizeof device model number and string terminator
    char device_firmware_revision[DECIMAL_DIGITS_MAX(uint8_t) + // Sizeof versions two dots and string terminator
                                  DECIMAL_DIGITS_MAX(uint8_t) + DECIMAL_DIGITS_MAX(ble_version.subversion_number) +
...
#ifdef DEBUG
    sprintf(device_firmware_revision,
            "%u.%u.%u.%u",
            DEVICE_APP_ID,
            DEVICE_APP_V,
            ble_version.subversion_number,
            (uint8_t)p_bootloader_data->bootloader_version);
#else
    sprintf(device_firmware_revision,
            "%u.%u.%u.%u",
            DEVICE_APP_ID,
            (uint8_t)p_bootloader_data->application_version,
            ble_version.subversion_number,
            (uint8_t)p_bootloader_data->bootloader_version);
#endif
