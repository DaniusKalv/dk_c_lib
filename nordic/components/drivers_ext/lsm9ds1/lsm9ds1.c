...
bool lsm9ds1_init(lsm9ds1_t * p_lsm9ds1) 
{
	...
	if(twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_ACC_GYR_WHO_AM_I_REG, &data, sizeof(data))){
		if(data == LSM9DS1_ACC_GYR_WHO_AM_I){
			NRF_LOG_INFO("Success identifying LSM9DS1 accelerometer & gyro at i2c address: 0x%x", p_lsm9ds1->acc_gyr_i2c_address);
		}
		else{
			return false;
		}
		...
		NRF_LOG_WARNING("Could not identify LSM9DS1 accelerometer & gyro at i2c address: 0x%x", p_lsm9ds1->acc_gyr_i2c_address);
		return false;
	}
...
//-----------------------Acc---Gyro---Config---------------------------
	...
	if(!write_acc_gyr_reg(p_lsm9ds1, LSM9DS1_CTRL_REG3_G, LSM9DS1_MASK_REG3_G_LP_MODE)){ // Enable gyro low power mode
		return false;
	}
...
//--------------------MAG---config---------------------------------------
	if(!write_mag_reg(p_lsm9ds1, LSM9DS1_CTRL_REG5_M, LSM9DS1_MASK_REG5_M_BDU)){ // Enable BDU
		return false;
	}
...
bool lsm9ds1_reset(lsm9ds1_t * p_lsm9ds1){
	if(!write_acc_gyr_reg(p_lsm9ds1, LSM9DS1_CTRL_REG8, LSM9DS1_MASK_REG8_SW_RESET)){ // Soft reset accelerometer
		return false;
	}
...
{
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_OUT_X_L_XL, (uint8_t *)p_lsm9ds1_acc_data, sizeof(lsm9ds1_acc_data_t));
}

bool lsm9ds1_read_gyr(lsm9ds1_t * p_lsm9ds1, lsm9ds1_gyr_data_t * p_lsm9ds1_gyr_data)
{
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_OUT_X_L_G, (uint8_t *)p_lsm9ds1_gyr_data, sizeof(lsm9ds1_gyr_data_t));
}

bool lsm9ds1_read_mag(lsm9ds1_t * p_lsm9ds1, lsm9ds1_mag_data_t * p_lsm9ds1_mag_data)
{
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->mag_i2c_address, LSM9DS1_OUT_X_L_M, (uint8_t *)p_lsm9ds1_mag_data, sizeof(lsm9ds1_mag_data_t));
}

bool lsm9ds1_read_acc_gyr_status(lsm9ds1_t * p_lsm9ds1, uint8_t * status)
{
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_STATUS_REG, status, sizeof(uint8_t));
}

bool lsm9ds1_read_acc_int_src(lsm9ds1_t * p_lsm9ds1, uint8_t * int_src){
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_INT_GEN_SRC_XL, int_src, sizeof(uint8_t));
}

bool lsm9ds1_read_gyro_int_src(lsm9ds1_t * p_lsm9ds1, uint8_t * int_src){
	return twi_read(p_lsm9ds1->i2c_instance, p_lsm9ds1->acc_gyr_i2c_address, LSM9DS1_INT_GEN_SRC_G, int_src, sizeof(uint8_t));
}
...
bool lsm9ds1_acc_power_down(lsm9ds1_t * p_lsm9ds1){
	if(!(p_lsm9ds1->sensor_status & LSM9DS1_GYR_ENABLED)){ // If gyro not enabled
		if(!write_acc_gyr_reg(p_lsm9ds1, LSM9DS1_CTRL_REG1_G, LSM9DS1_MASK_REG1_G_POWER_DOWN)){
			return false;
		}
	}
	else{
		if(!lsm9ds1_acc_out_enable(p_lsm9ds1, false)){ // Disable accelerometer data output
			return false;
		}
	}
...
bool lsm9ds1_gyr_power_down(lsm9ds1_t * p_lsm9ds1){ // This function disables both the gyro and accelerometer
...
bool lsm9ds1_set_mag_alert_threshold(lsm9ds1_t * p_lsm9ds1, uint16_t threshold){
	...
	if(!write_mag_reg(p_lsm9ds1, LSM9DS1_INT_THS_H, (threshold & 0x7F))){ // Set MSB to 0 as specified in the datasheet
		return false;
	}
	return true;
}
...
bool lsm9ds1_enable_acc_alert_int(lsm9ds1_t * p_lsm9ds1, bool enable){
	if(!write_acc_gyr_reg(p_lsm9ds1, LSM9DS1_INT_GEN_DUR_XL, 0x00/*LSM9DS1_MASK_INT_GEN_DUR_WAIT_XL | 4*/)){ // Enable accelerometer wait function (1 samples before exiting the interrupt)
		return false;
	}
...
bool lsm9ds1_set_gyro_alert_threshold(lsm9ds1_t * p_lsm9ds1, int16_t threshold){
	...
	if(threshold > INT_15_BIT_MAX) threshold = INT_15_BIT_MAX;
	else if (threshold < INT_15_BIT_MIN) threshold = INT_15_BIT_MIN;
	...
	//threshold &= ~LSM9DS1_MASK_INT_GEN_THS_XH_G_DCRM; // Set DCRM bit to zero as the following registers don't contain it 
...
bool lsm9ds1_enable_gyro_alert_int(lsm9ds1_t * p_lsm9ds1, bool enable){
	if(!write_acc_gyr_reg(p_lsm9ds1, LSM9DS1_INT_GEN_DUR_G, 0x00)){ // Gyro no wait
		return false;
	}
